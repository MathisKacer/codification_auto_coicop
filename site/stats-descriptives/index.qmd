...
détaillent par division. Les divisions 98 et 99 sont des codes
...
division COICOP vraie (rappel), pour chaque niveau de troncature. Comme dans
la section précédente, les divisions 98 et 99 sont traitées comme n'importe
quelle autre division, sans exclusion.
...
def accuracy_llm_par_division(niveau):
    ...
    recap["n_erreurs"] = recap["n"] - recap["n_correct"]
    return recap[["n", "n_correct", "n_erreurs", "accuracy"]]

titre("Accuracy du LLM-judge par division (niveau 4)")
...
```{python}
titre("Accuracy du LLM-judge par division (niveau 3)")
...
```{python}
titre("Accuracy du LLM-judge par division (niveau 2)")
...
```{python}
titre("Accuracy du LLM-judge par division (niveau 1)")
