...
# =============================================================================
# Accuracy ventilée par un groupe arbitraire (mode de collecte, prix...)
# =============================================================================

def accuracy_par_groupe(df, col_pred, col_vrai, groupe, niveau=4, verbose=True):
    """
    Accuracy de `col_pred` contre `col_vrai` au niveau demandé, ventilée par
    un groupe arbitraire plutôt que par division COICOP (cf.
    `accuracy_par_division`) : mode de collecte (`source`), tranche de
    budget...

    Parameters
    ----------
    groupe : pd.Series
        Variable de regroupement, alignée sur l'index de `df` (ex.
        `df["source"]`, ou une tranche de budget déjà découpée via
        `pd.cut(df["budget"], ...)`). Les valeurs manquantes doivent déjà
        être remplacées par une catégorie explicite (ex. "INCONNU") avant
        l'appel : `groupby` les exclurait sinon silencieusement.

    Returns
    -------
    pd.DataFrame indexé par valeur du groupe (+ une ligne "TOTAL"), colonnes
    ...
    out["pred_tronq"] = out[col_pred].map(lambda x: tronquer_niveau(x, niveau))
    out["correct"] = out["pred_tronq"] == out["vrai_tronq"]
    out["_groupe"] = groupe.astype(str).values

    def _resume(g):
        return pd.Series({
            "n": len(g), "n_correct": int(g["correct"].sum()), "accuracy": g["correct"].mean(),
        })

    recap = out.groupby("_groupe", observed=True).apply(_resume, include_groups=False)
    recap.loc["TOTAL"] = _resume(out)
    recap.index.name = groupe.name or "groupe"
    recap[["n", "n_correct"]] = recap[["n", "n_correct"]].astype(int)

    if verbose:
        print(f"=== Accuracy {col_pred} vs {col_vrai} au niveau {niveau}, "
              f"par {recap.index.name} ===")
        print(recap.to_string(formatters={"accuracy": "{:.1%}".format}))

    return recap


def accuracy_multi_classifieurs_par_groupe(df, cols_pred, col_vrai, groupe, niveau=4, verbose=True):
    """
    Comme `accuracy_multi_classifieurs`, mais la ventilation se fait par un
    groupe arbitraire (mode de collecte, tranche de budget...) plutôt que par
    division COICOP. Voir `accuracy_par_groupe` pour le format de `groupe`.

    Returns
    -------
    pd.DataFrame indexé par valeur du groupe (+ une ligne "TOTAL"), colonnes :
        - n              : effectif du groupe (identique pour tous les
                            classifieurs, basé sur le vrai code)
        - une colonne par élément de `cols_pred` : accuracy de ce
          classifieur sur ce groupe
    """
    resultats = {}
    n_groupe = None
    for col in cols_pred:
        recap = accuracy_par_groupe(
            df, col_pred=col, col_vrai=col_vrai, groupe=groupe, niveau=niveau, verbose=False,
        )
        resultats[f"accuracy_{col}"] = recap["accuracy"]
        if n_groupe is None:
            n_groupe = recap["n"]

    out = pd.DataFrame({"n": n_groupe, **resultats})
    out.index.name = groupe.name or "groupe"

    if verbose:
        print(f"=== Accuracy comparée ({', '.join(cols_pred)}) vs {col_vrai} "
              f"au niveau {niveau}, par {out.index.name} ===")
        print(out.to_string(formatters={c: "{:.1%}".format for c in resultats}))
